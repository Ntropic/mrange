...
def Make_U_Partial(U_1, U_0):
    ...
    return E0, V0
#@njit(fastmath=True, nogil=True, cache=True)
def Make_U_Partial_3(U_1, U_0):
    s = U_1.shape[0]
    D0 = 1j*logmU(dot( dot(U_1, U_0), U_1))
    E0, V0 = eigh(D0)
    return E0, V0

##### Unitaries ########################################################################################################
@njit(fastmath=True, nogil=True, cache=True)
def expmH_2D(s, L, EL, CL, EC, CR, ER, R, i_a, i_b, i_c, c_a, c_b):
    A = empty((s, s), complex128)
    A2 = empty((s, s), complex128)
    A3 = empty((s, s), complex128)
    sb = int((sign(c_b)-1)/2)
    sa = int((sign(c_a)-1)/2)
    i_r = i_b + sb
    i_l = i_a + sa
    ind = -2*sb-sa
    for i in range(s):
        A[i, :]  = exp(-1j * ER[i_b, i_l, i] * abs(c_a)) * R[i_b, i_l, i, :]
        A2[i, :] = exp(-1j * EC[i_r, i_a, i] * abs(c_b)) * CR[i_b, i_c, ind, i, :]
        A3[i, :] = exp(-1j * EL[i_b, i_l, i] * abs(c_a)) * CL[i_b, i_c, ind, i, :]
    return dot(L[i_b, i_l, ...], dot(A3, dot(A2, A)))
